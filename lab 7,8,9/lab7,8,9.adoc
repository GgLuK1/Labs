...
Данный виртуальный класс содержит интерфейс работы кнопки.
...
Данный класс реализует интерфейс работы светодиодов.
...
Виртуальный класс, поскольку для построения модов в функции должно меняться значение индекса, поэтому константой ее оставлять нельзя.
...
Данный класс отвечает за моргание светодиодов. Класс является шаблонным, поэтому нам необходимы два регистра:ODR и BSRR. Функция Toggle реализует мигание светодиодов, а функция Switchoff необходима для отключения светодиодов.
...
В данном классе используется функции описанные в классе Led для массива светодиодов. 
...
Данный класс отвечает за кнопку, с помощью которой реализуется переключение модов. Данный класс основан на булевой логике, также предусмотрена защита от дребезга, то есть при нажатии на кнопку программа не должна проскакивать несколько режимов, а работать корректно.
...
Данный класс реализует режим, когда все светодиоды загораются и гаснут одновременно.
...
Данный класс реализует режим "Ёлочка", то есть светодиоды загораются поочередно и гаснут одновременно.
...
Данный класс реализует режим "Шахматы", то есть загораются сначала первый и второй и гаснут, далее загораются третий, четвертый и гаснут соответственно.
...
Данный класс помогает нам осуществить переключение написанных выше режимов.С помощью изменения индекса мода на единицу.
...
В данном классе мы реализуем настройку генератора частоты, светодиодов и таймера как мы делали ранее.
